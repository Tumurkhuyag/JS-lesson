...
// Тухайн хүсүүсийн хүйс, болон BMR олох функцыг processArray функцруу дамжуулж гарсан утгыг нь bmr array -д хадгалах
var bmr = processArray(isMale, findBMR);
...
function processArray(myArr, fn) {
  var arr = [];
  for (i = 0; i < myArr.length; i++) {
    // "isMale" массив "myArr" -ын утгад дамжуулагдсан, "findBMR" функц "fn" функцын утгад дамжуулагдсан
    // "myArra[i]" дугаар элемент, болон "i" буюу i дугаар элементийг заагч тоог fn функцруу дамжуулан бололвсруулж, гарсан утгыг нь хүлээж аван "arr" массивт урдаас нь хойш нь залгах кодчилол
    arr.push(fn(myArr[i], i));
  }
  return arr;
}
...
// Men: BMR = 66 + (13.7 * weight in kg) + (5 * height in cm) - (6.8 * age in years)
function findBMR(el, i) {
  // Эрэгтэй бол ажиллах томъёо
  if (el)
    return Math.round(
      66 + 13.7 * bodyWeight[i] + 5 * bodyHeight[i] - 6.8 * ages[i]
    );
  // Эмэгтэй бол ажиллах томъёо
  else
    return Math.round(
      655 + 9.6 * bodyWeight[i] + 1.8 * bodyHeight[i] - 4.7 * ages[i]
    );
...
// Console дотор мэдээлэл хэвлэх төдий функц
function calorieAdvice() {
  for (i = 0; i < bmr.length; i++) {
    if (isMale[i]) person = "эрэгтэй";
    else person = "эмэгтэй";
